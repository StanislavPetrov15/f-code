  enum class SpatialKind { COMPLETE, SEGMENT };
    ...
    template<typename T> struct list
    {
        ...
        list<T>& RotateLeft(int _positions = 1)
        {
            ...
            list<T> accumulator = *this;
            list<T> copy = *this;

            for (int n = 0; n < _positions; n++)
            {
                accumulator[accumulator.count() - 1] = copy[0];

                for (int i = 0; i < Count - 1; i++)
                {
                    accumulator[i] = copy[i + 1];
                }
        ...
        list<T>& RotateRight(int _positions = 1)
        {
            ...
            for (int n = 0; n < _positions; n++)
            {
                accumulator[0] = copy[copy.count() - 1];
        ...
        list<list<T>> Multisplit(const list<T>& _set, bool _ignoreEmptyValues = false) const
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                ...
                {
                    accumulator.Append(Subrange(i, Count - 1));
                    break;
                }
            }
